	...
	LFD_bigMalesFemales<-LFD_bigMalesFemales[which(LFD_bigMalesFemales$Sex==1),]
	...
	LL <- Lengths[c(WHICH84, WHICH85JO,WHICH86FM)]				#Assign the number of individuals
	MM1984 <- as.numeric(MONTHS[WHICH84])-12
	MM1985 <- as.numeric(MONTHS[WHICH85JO])					#Assign January to be the 0th months
	...
	MM<-c(MM1984,MM1985,MM1986)									#Concatenate these months values together
	...
	while (LOGLIKE - LOGOLD > TOL)
	{
	  ...
	  BLAH <- 50:170
	  plot(BLAH,exp(VV[1])+VV[2]*BLAH+VV[3]*BLAH^2,type='l',xlab="Mean length (mm)", ylab="Variance",lwd=2.5,ylim=c(0,1300))
