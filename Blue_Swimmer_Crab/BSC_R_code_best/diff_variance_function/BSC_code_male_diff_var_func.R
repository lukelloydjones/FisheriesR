...
LFD_bigFem<-LFD_bigMalesFemales[which(LFD_bigMalesFemales$Sex==1),]
...
WHICH86FM <- which((YEAR == '1986') & (as.numeric(MONTHS) %in% (2:5)))
	...
	K0 <- 0.7														#Initialise K0 average K
	LINF <- 180												#Initialise asym length
	M01<-65														#Initialise first month's average length
	M02<-80														#Initialise second month's average length
	T0 <- 0.9													#Initialise first seasonality parameter
	T1 <- 0.05														#Initialise second seasonality parameter
    VV=c(29.51196695,0.02469080,-16.11090706,804.95058722)										   #Initialise the starting values for the variance quadratic update
	
	
	#Setting values for drawing plots 
	# 0.69186831   0.86693179   0.04361348 182.24072885  71.44926516  79.15985997  
	#71.58270906   0.03171859 -13.62674639 945.59345409
#	K0=0.69186831;  LINF=182.24072885 ; M01=71.44926516; M02=79.15985997; T0=0.86693179; T1=0.04361348; VV=c( 71.58270906,   0.03171859, -13.62674639, 945.59345409)
	...
	while (LOGLIKE - LOGOLD > TOL)
	{
	  ...
	  # Define the optimal constraint value
	  M=0.008
	  
	  # Define the first optim function
	  
	  PARA <- c(K0,T0,T1,LINF,M01,M02)
	  OPTIFUN <- function(PARA)
	  {
	  	PARA[2]=PARA[3]*cos(2*pi*M)/(sqrt(1-cos(2*pi*M)^2))
	  	#PARA[1]=0.6
	  	LOGLIKE <- 0
	  		for (mm in 1:MON)
	  			{
	  ...
	  #T0 <- OPTIM$par[2]
	  T1 <- OPTIM$par[3]
	  T0=T1*cos(2*pi*M)/(sqrt(1-cos(2*pi*M)^2))
	  LINF <- OPTIM$par[4]
	  M01 <- OPTIM$par[5]
	  M02<-OPTIM$par[6]
	  ...
	  #VV=c(3,150,100)
	  BLAH <- 0:180
	  var_linf=VV[1]*BLAH*exp(-VV[2]*BLAH)+exp(VV[3]*(1-exp(-VV[4]*BLAH)))
	  plot(BLAH,var_linf,type='l',xlab="Mean length (mm)", ylab="Variance",lwd=2.5)
	  plot(density(rnorm(1000000, LINF,sqrt(VV[1]*LINF*exp(-VV[2]*LINF)+exp(VV[3]*(1-exp(-VV[4]*LINF)))  ))),xlab="Length (mm)",main="",lwd=2.5)
	}
