	...
	setwd("~/Dropbox/AAUni/APhD/Blueswimmer/RcodeData_03:10:2014/Plus_Large/")
	
	#Process the new larger ones
	#new_large=read.delim(pipe("pbpaste"),header=T)
	#write.table(new_large,"BSC_Large_99_00")
	...
	#Read in the largest males 
	
	new_large=read.table("BSC_Large_99_00",header=T)
	
	#Take a smaller sample to see if it works first
	
	new_large = new_large[sample(seq(0,dim(new_large)[1]),5000),]
	
	Dates_Large_Male<-as.Date(new_large$Date,"%d/%m/%Y")               
	Lengths_Large_Male<-new_large$Carapace.width
	...
	Dates <- c(Dates2,Dates1, Dates_Large_Male)
	Lengths <- c(Lengths2,Lengths1,Lengths_Large_Male)
	...
	WHICH99   <- which((YEAR == '1999') & (as.numeric(MONTHS) %in% (Start_Month:End_Month_Yr1)))
	WHICH00   <- which((YEAR == '2000') & (as.numeric(MONTHS) %in% (Start_Month:End_Month_Yr2)))
	WHICH84   <- which((YEAR == '1984') & (as.numeric(MONTHS) %in% (12)))
	...
	MON <-length(c(12,Start_Month: End_Month_Yr1,Start_Month:End_Month_Yr2))		#Assign how many months we would like to model
	LL <- Lengths[c(WHICH84, WHICH85JO, WHICH86FM, WHICH99, WHICH00 )]				#Assign the number of individuals
	MM1984 <- as.numeric(MONTHS[WHICH84])-12
	MM1985 <- as.numeric(MONTHS[WHICH85JO])				#Assign Decemeber to be the 0th month. Recruitment begins
	MM1986 <- as.numeric(MONTHS[WHICH86FM])+12				#Thus January of next year will be the 12 month. Plus we do this so that we don't have to estimate yr
	MM1999 <- as.numeric(MONTHS[WHICH99])
	MM2000 <- as.numeric(MONTHS[WHICH00])+12
	MM<-c(MM1984,MM1985,MM1986, MM1999, MM2000)									#Concatenate these months values together
	...
	while (LOGLIKE - LOGOLD > TOL)
	{
	  ...
	  par(mfrow = c(4,5))
	  XX <- 0:200
	  xlabNam<-c("Dec., 1984","Jan., 1985","Feb., 1985","Mar., 1985","Apr., 1985","May, 1985","Jun., 1985","Jul, 1985","Aug., 1985","Sep., 1985","Oct., 1985","Nov., 1985","Dec., 1985","Jan., 1986","Feb., 1986","Mar., 1986","Apr., 1986","May, 1986")
	  for (mm in 1:MON)
	  {
...
### DRAW A HISTOGRAM OF THE LARGER VALUES TO GET AN IDEA OF THE SIZE DISTRIBUTION
	...
	Dates <-  Dates_Large_Male
	Lengths <-Lengths_Large_Male
	...
	#Pull out the years and months that we a re interested in i.e., those that don't contain recruitment
	WHICH99   <- which((YEAR == '1999') & (as.numeric(MONTHS) %in% (Start_Month:End_Month_Yr1)))
	WHICH00   <- which((YEAR == '2000') & (as.numeric(MONTHS) %in% (Start_Month:End_Month_Yr1)))
	
	
	#INITIALISE
	
	MON <-24		#Assign how many months we would like to model
	LL <- Lengths[c(WHICH99, WHICH00 )]				#Assign the number of individuals
	MM1999 <- as.numeric(MONTHS[WHICH99])
	MM2000 <- as.numeric(MONTHS[WHICH00])+12
	MM<-c(MM1999, MM2000)									#Concatenate these months values together
	MMLIST <- as.numeric(names(table(MM)))						#Make a list of the months from January first i.e., January 1986 is gets assigned a 12

	par(mfrow=c(4,6))
	  for (mm in 1:MON)
	  {
	    hist(LL[which(MM==MMLIST[mm])],breaks=30,prob=T,xlim=c(20,210),ylim=c(0,0.055),xlab="Month",ylab="",main=round(mean(LL[which(MM==MMLIST[mm])],3)))
	    #print(mean(LL[which(MM==MMLIST[mm])]))
	    #lines(XX,PI1[mm]*dnorm(XX,MU2A(mm,K0,T0,T1,LINF,M01,M02),sqrt(VAR2A(mm,VV,K0,T0,T1,LINF,M01,M02))),col='red',lwd=2.5)
	    #ines(XX,PI2[mm]*dnorm(XX,MUA(mm,K0,T0,T1,LINF,M01,M02),sqrt(VARA(mm,VV,K0,T0,T1,LINF,M01,M02))),col='green',lwd=2.5)
	    #lines(XX,(1-(PI1[mm]+PI2[mm]))*dnorm(XX,MUJ_AUX[mm],sqrt(VARJ(mm,VV,K0,T0,T1,LINF,M01,M02))),col='blue',lwd=2.5)
	  }
